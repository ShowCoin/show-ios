...
@implementation NSDate (TSJ)

-(NSString *)dateStringByCircle{
    NSInteger year = [self year];
    NSInteger moth = [self month];
    NSInteger day = [self day];
    return [NSString stringWithFormat:@"%02ld%02ld月%04ld年",day,moth,year];
}

- (NSString *)agoString {
    NSDate * now = [NSDate date];
    
    double elapsed = [now timeIntervalSinceDate:self];
    
    if ( elapsed > 3600*24 ){
        return [NSString stringWithFormat:@"%d天前",(int)(elapsed/(3600*24))];
    }
    
    if ( elapsed > 3600 ){
        return [NSString stringWithFormat:@"%d小时前",(int)(elapsed/(3600))];
    }
...
-(NSString *)formattedTime{
    ...
    if (!hasAMPM) { //24小时制
        ...
        if (hour >= 0 && hour <= 6) {
            ...
            hour = hour%24;
            if (hour >= 0 && hour <= 6) {
                dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"MM-dd 凌晨hh:mm"];
            }else if (hour > 6 && hour <=11 ) {
                dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"MM-dd 上午hh:mm"];
            }else if (hour > 11 && hour <= 17) {
                dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"MM-dd 下午hh:mm"];
            }else if (hour > 17 && hour <= 24) {
                dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"MM-dd 晚上hh:mm"];
            }else if (hour < 0 && hour >= -24){
                dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"MM-dd HH:mm"];
            }else{
                dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"yyyy-MM-dd HH:mm"];
 ...
 *  按天
 */
- (NSString *)preciseDayDateDescription {
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    NSString *theDay = [dateFormatter stringFromDate:self];//日期的年月日
    NSString *currentDay = [dateFormatter stringFromDate:[NSDate date]];//当前年月日
...
- (NSString *)preciseDayDateDescription1 {
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    NSString *theDay = [dateFormatter stringFromDate:self];//日期的年月日
    NSString *currentDay = [dateFormatter stringFromDate:[NSDate date]];//当前年月日
...
+ (NSDate *) dateWithSecondFromNow: (NSInteger) dSecond
{
    NSTimeInterval aTimeInterval = [[NSDate date] timeIntervalSinceReferenceDate] + dSecond;
    NSDate *newDate = [NSDate dateWithTimeIntervalSinceReferenceDate:aTimeInterval];
    return newDate;
}
...
- (BOOL) isSameWeekAsDate: (NSDate *) aDate
{
    ...
    return (labs((NSInteger)[self timeIntervalSinceDate:aDate]) < D_WEEK);
}
...
+(NSString *)hmsWithSeconds:(long)seconds{
    NSString *time = @"00:00:00";
    if (seconds >0) {
        long t = seconds /1000;
        long d = (t / 24/60/60);
        long h = (t/3600);
        ...
        if (d > 0) {
            time = [NSString stringWithFormat:@"%ld天",d];
        }
    }
    return time;
}
