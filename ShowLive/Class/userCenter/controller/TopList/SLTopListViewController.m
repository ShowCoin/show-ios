...
- (void)viewDidLoad {
    ...
    [contributionBtn setTitle:@"贡献榜" forState:UIControlStateNormal];
    ...
    [encourageBtn setTitle:@"激励榜" forState:UIControlStateNormal];
...
- (UIScrollView *)bkscrollerView{
    if (!_bkscrollerView) {
        ...
        [_bkscrollerView setContentOffset:CGPointMake(kMainScreenWidth, 0) animated:NO];
...
-(SLToplistSubView *)contributionView
{
    if (!_contributionView) {
        _contributionView = [SLToplistSubView authViewWithFrame:CGRectMake(0, 0, kMainScreenWidth, kMainScreenHeight - KNaviBarHeight)];
...
{
    if (!_encourageView) {
        _encourageView = [SLToplistSubView authViewWithFrame:CGRectMake(kMainScreenWidth, 0, kMainScreenWidth, kMainScreenHeight - KNaviBarHeight)];
