 ...
 *  自定义的button
 */
@interface MyButton : ShowHitEdgeExpandButton
...
- (void)setCloseButtonType:(ButtonPositionType)closeButtonType {
    ...
    self.closeButton.expandHitEdge = UIEdgeInsetsMake(40, 40, 40, 40);
}

- (void)showMiniCardView:(UIView *)presentView originPoint:(CGPoint)originPoint {
    ...
    viewController.view.backgroundColor = [[UIColor grayColor] colorWithAlphaComponent:0.5];//self.popBackgroudColor;
    self.viewController = viewController;
    
    CGRect containerViewRect = presentView.bounds;
    containerViewRect.origin = originPoint;
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [UIView animateWithDuration:kTransformPart1AnimationDuration animations:^{
            CGRect newRect = containerViewRect;
            newRect.origin.x -= containerViewRect.size.width;
...
- (void)showGiftTipsWithPresentView:(UIView *)presentView
{
    [HWPopTool sharedInstance].shadeBackgroundType = ShadeBackgroundTypeClear;
    [HWPopTool sharedInstance].closeButtonType = ButtonPositionTypeNone;
    [self showWithPresentView:presentView animated:YES hasAlphaMask:YES];
}

- (void)showWithPresentView:(UIView *)presentView animated:(BOOL)animated {

    [self showWithPresentView:presentView animated:animated hasAlphaMask:NO];
}

- (void)showWithPresentView:(UIView *)presentView animated:(BOOL)animated hasAlphaMask:(BOOL)hasAlphaMask
{
    self.window = [[UIWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    self.window.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
    self.window.opaque = NO;
    ...
    if (ButtonPositionTypeNone != self.closeButtonType) {
...
- (void)close {
    [self hideAnimated:NO withCompletionBlock:nil];
    
}
- (void)closeWithBlcok:(void(^)())complete {
    [self hideAnimated:NO withCompletionBlock:complete];
}
...
- (void)drawRect:(CGRect)rect{
    ...
    }else if (ShadeBackgroundTypeClear == [HWPopTool sharedInstance].shadeBackgroundType){
