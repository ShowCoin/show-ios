...
#import "SecurityStoreUtils.h"

@implementation SecurityStoreUtils
+ (void)rhKeyChainSave:(id)data withServiceKey:(NSString*)serviceKey {
    [self save:serviceKey data:data];
}

+ (id)rhKeyChainLoad:(NSString*)serviceKey{
    id data = [self load:serviceKey];
    return data;
}

+ (void)rhKeyChainDelete:(NSString *)serviceKey{
    [self delete:serviceKey];
}
