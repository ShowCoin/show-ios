...
@interface SLGooglePassWordAlert ()<UITextFieldDelegate>
...
@implementation SLGooglePassWordAlert
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        // 输入安全密码的背景View
        UIView *BGView = [[UIView alloc] initWithFrame:CGRectMake((kMainScreenWidth - 280*Proportion375) / 2, 0, 280*Proportion375, 142*Proportion375)];
        ...
        [cancelButton setBackgroundImage:[UIImage imageNamed:@"live_chat_close"] forState:UIControlStateNormal];
        ...
        // 密码框
        CGFloat passwordTextFieldY = topLineView.bottom + 25*Proportion375;
...
- (void)sureButtonAction {
    ...
    if ([self.delegate respondsToSelector:@selector(sureActionWithAlertGooglePasswordView:password:)]) {
        [self.delegate sureActionWithAlertGooglePasswordView:self password:self.passwordTextField.text];
    }
}
